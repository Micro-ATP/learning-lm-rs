...
1. 项目介绍
继续使用方向三作业阶段代码（一个用Rust编写的简单的大模型推理系统），完成大模型推理所需的全部代码，支持基于llama模型的文本生成和AI对话
2. 项目目标
学员通过该项目，学习大模型推理的相关知识、理解大模型的基本原理、了解大模型推理系统的一些挑战和优化方向。完成该项目的学员，应该基本具备独立手写大模型推理的计算代码、搭建简单大模型推理服务的能力。
3. 时间安排
截止至 9 月 29 日晚结营仪式前。
4. 完成标准
及格标准：完成文本生成功能
优秀标准：完成单机多轮AI对话功能，（需要有一个支持用户输入的UI，可以是命令行），并至少为推理系统实现以下拓展功能中的一种：
实现FP16半精度推理
实现多会话单机AI对话助手，支持对话之间的切换，历史对话撤销重推理，UI实现方式不限
实现支持多用户请求的AI对话服务网络API，可选择不存储之前请求的历史信息
实现CPU上的分布式推理（可使用OpenMP、MPI等），对比大模型各层计算的延迟
其他优化，请实现和导师商议

学员需提交自己的项目地址，并包含一个最终报告（可以是markdown，pdf等形式），内容包括自己实现的功能的简单说明和实际效果截图。
作业提交可以发导师邮箱：panzezhong@qiyuanlab.com
